...
# 追加：圖片切片可拖曳切線（2026-08-11）

## 階段 15：圖片切片不等寬長方形
- `static/image-slicer/index.html` 將等分切點改為 `xCuts` / `yCuts` 邊界陣列，初始仍平均分配。
- 預覽格線改為可拖曳的垂直 / 水平切線，拖曳時限制在相鄰切線之間，避免切片重疊或掉像素；支援觸控指標與鍵盤方向鍵微調。
- 欄數 / 列數變動時保留既有切線，增加欄列從最寬區塊插入，減少欄列合併最窄相鄰區塊。
- Canvas 切割直接使用拖曳後的邊界，PNG / JPG / WEBP、去綠幕與 HEIC 輸入沿用既有流程。
- 寬度 521 至 880 px 時維持左側控制、右側預覽，只有窄手機才上下堆疊；中等寬度的預覽改為靠上顯示，避免主要工作區被推到下方。

### Preflight Checklist
- [x] 不修改資料庫、權限、API、第三方套件或使用者檔案上傳路徑，圖片仍只在瀏覽器處理。
- [x] 不涉及 SSRF、個資、金錢、時區、對外通知或跨服務副作用。
- [x] 拖曳與鍵盤操作皆限制切點範圍，確保每個切片邊界連續。
- [x] 以腳本驗證切點資料模型與不等寬邊界，再以 Chrome 實測拖曳垂直 / 水平切線、鍵盤微調、欄數調整與 ZIP 完成狀態。

**狀態**：完成。Chrome 實測 1200 × 800 測試圖，垂直切線由 400 px 拖到 180 px、水平切線由 266 px 拖到 160 px，畫面與 ARIA 數值同步，切成 9 塊並完成 ZIP 下載；欄數增加至 4 時保留既有切線並從最寬區塊新增切線。另以 577 px 寬度確認預覽位於右側較寬區域，390 px 寬度仍維持上下堆疊。

# 追加：圖片壓字工具（2026-08-12）

## 階段 16：圖片壓字工具 `/text-watermark`

**目標**：批次上傳圖片，輸入文字並調整顏色、大小、角度、位置與透明度，可選擇重複鋪滿整張圖片，最後下載單張圖片或全部 ZIP。

- 純前端實作，圖片不上傳伺服器。
- 新增 `static/text-watermark/index.html`，沿用 `.tool-layout`、`.dropzone`、`.range-field`、`.message` 與亮 / 暗模式。
- 支援 JPG、PNG、WEBP、AVIF 輸入，以及沿用 `static/shared/heic-decode.js` 的 HEIC / HEIF 解碼。
- 多張圖片共用同一組文字設定，右側縮圖可切換目前預覽圖片。
- 文字設定包含內容、顏色、大小、角度、透明度、水平位置、垂直位置與重複鋪滿。
- 文字大小以圖片短邊比例保存，位置以圖片寬高比例保存，避免批次套用到不同尺寸時失真或跑位。
- 預覽與下載共用 `window.TextWatermark.renderWatermarkedImage`，確保畫面與輸出一致。
- 單張下載沿用原始格式；HEIC / HEIF 輸出 JPEG，AVIF 輸出 PNG；全部下載使用既有 JSZip CDN 打包。
- 新增 `GET /text-watermark` 路由、首頁工具卡片與 README 工具說明。

### Preflight Checklist

- [x] 圖片只在瀏覽器內以 Canvas 處理，不進入後端 API、資料庫或第三方服務。
- [x] 顯示檔名與錯誤內容使用 DOM `textContent`，不將使用者輸入當作 HTML。
- [x] 限制支援格式與單張 50 MP 上限，避免解碼異常圖片造成過量記憶體使用。
- [x] 清除圖片與離開頁面時釋放 object URL。
- [x] 批次輸出逐張處理並讓出主執行緒，單張失敗不會吞掉錯誤。
- [x] ZIP 檔名移除路徑分隔符與特殊字元。
- [x] 不新增第三方套件，沿用既有 Canvas、HEIC 解碼器與 JSZip。
- [x] 不涉及資料庫、權限、CSRF、SSRF、時區、對外通知或跨服務副作用。

**狀態**：完成。已以 Chrome 實測兩張圖片上傳、縮圖切換、文字樣式、拖曳定位、重複鋪滿、單張下載與全部 ZIP 下載；pytest 整合測試 4 passed。